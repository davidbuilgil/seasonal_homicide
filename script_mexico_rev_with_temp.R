...
temperature <- read.csv(here('data/mexico-replication/cmip6-x0.25_timeseries_tas_timeseries_monthly_1950-2014_mean_historical_hadgem3-gc31-ll_r1i1p1f3_mean.csv'))
...
# Filter out Mexico (country) from temperature database
temperature <- temperature %>%
  filter(name != "Mexico")
...
state_lookup3 <- tibble::tribble(
  ...
  "Chiapas", "CHIAPAS",
  "Chihuahua", "CHIHUAHUA",
  "Ciudad de M\x82xico", "CIUDAD DE MEXICO",
  "Durango", "DURANGO",
  "Guanajuato", "GUANAJUATO",
  "Guerrero", "GUERRERO",
  "Hidalgo", "HIDALGO",
  ...
  "Morelos", "MORELOS",
  "Nayarit", "NAYARIT",
  "Nuevo Le\xa2n", "NUEVO LEON",
  "Oaxaca", "OAXACA",
  "Puebla", "PUEBLA",
  "Quer\x82taro", "QUERETARO",
  "Quintana Roo", "QUINTANA ROO",
  "San Luis Potos\xa1", "SAN LUIS POTOSI",
  "Sinaloa", "SINALOA",
  "Sonora", "SONORA",
  "Tabasco", "TABASCO",
  "Tamaulipas", "TAMAULIPAS",
...
temperature_state <- temperature %>%
  left_join(state_lookup3, by = "name") %>%
  dplyr::select(year, month, ENTIDAD, value) %>%
  rename(temperature = value)

# Calculate monthly mean and deviantion from state and monthly mean across years
# Step 1: Calculate mean temperature for each month across years
temperature_state_mean <- temperature_state %>%
  group_by(month, ENTIDAD) %>%
  summarise(mean_temperature = mean(temperature, na.rm = TRUE))

# Step 2: Identify deviation from the mean and direction
temperature_state <- temperature_state %>%
  left_join(temperature_state_mean, by = c("month", "ENTIDAD")) %>%
  mutate(
    temperature_deviation = temperature - mean_temperature,  # Deviation size
    above_below_avg = ifelse(temperature_deviation > 0, "Above", "Below")
  )

# Merge rainfall, homicide and temperature state data
merged_state_data <- merged_state_data %>%
  left_join(temperature_state, by = c("year", "month", "ENTIDAD")) %>%
  select(year, month, ENTIDAD, rfh, mean_rfh, rainfall_deviation, 
...
fe_state_model <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation + 
                        factor(ENTIDAD), 
...
# Checking model
fe_state_model_2 <- lm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation + 
                         factor(ENTIDAD) + factor(year) + factor(month), 
                     data = merged_state_data %>% filter(year != 2025))
...
fe_state_model_3 <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation, 
                     data = merged_state_data %>% filter(year != 2025), 
