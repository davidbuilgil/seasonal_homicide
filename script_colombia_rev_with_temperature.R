...
temperature <- read.csv(here('data/cmip6-x0.25_timeseries_tas_timeseries_monthly_1950-2014_mean_historical_hadgem3-gc31-ll_r1i1p1f3_mean.csv'))
...
# Select national mean temperature
temperature_national <- temperature %>%
...
temperature_national_mean <- temperature_national %>%
  group_by(month) %>%
  summarise(mean_temperature = mean(value, na.rm = TRUE))

# Step 2: Identify deviation from the mean and direction
temperature_national <- temperature_national %>%
  left_join(temperature_national_mean, by = "month") %>%
  mutate(
...
merged_data <- merged_data %>%
  inner_join(temperature_national, by = c("year", "month")) %>%
  select(year, month, rainfall_deviation, homicide_deviation, deviation) %>%
...
# Fixed Effects Model
fe_model <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation, 
...
fe_model_check <- lm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation + 
...
gls_model <- gls(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation, 
                 correlation = corAR1(form = ~ year | month), 
                 data = merged_data %>% filter(year != 2025))

gls_model_ar2 <- gls(
  scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation,
...
# Filter out Colombia (country) from temperature database
temperature <- temperature %>%
  filter(name != "Colombia")

# Load geographic lookup table
geo_lookup <- read.csv(here('data/colombia_geo_lookup.csv'))

# Merge temperature data with lookup table
temperature_muni <- geo_lookup %>%
  left_join(temperature, by = "name")

# Calculate monthly mean and deviantion from municipality and monthly mean across years
# Step 1: Calculate mean temperature for each month across years
temperature_muni_mean <- temperature_muni %>%
  group_by(month, MPIO_CCDGO) %>%
  summarise(mean_temperature = mean(value, na.rm = TRUE))

# Step 2: Identify deviation from the mean and direction
temperature_muni <- temperature_muni %>%
  left_join(temperature_muni_mean, by = c("month", "MPIO_CCDGO")) %>%
  mutate(
    temperature_deviation = value - mean_temperature, 
    above_below_avg = ifelse(temperature_deviation > 0, "Above", "Below")
  ) %>%
  rename(COD_MUNI = MPIO_CCDGO)

# Merge rainfall, homicide and temperature municipality data
merged_muni_data <- merged_muni_data %>%
  left_join(temperature_muni, by = c("year", "month", "COD_MUNI")) %>%
  select(year, month, COD_MUNI, rfh, mean_rfh, rainfall_deviation, 
         standardized_homicide, mean_standardized_homicide, homicide_deviation,
         mean_temperature, temperature_deviation)
...
fe_dept_model <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation + 
                       factor(COD_MUNI), 
                     data = merged_muni_data %>% filter(year != 2025), 
                     index = c("year", "month"), 
                     model = "within")

summary(fe_dept_model) # coefficients show how much municipalities contribute towards change in homicide
summary(fe_dept_model)$coefficients["scaled_rainfall_deviation", "Estimate"]
summary(fe_dept_model)$coefficients["scaled_rainfall_deviation", "Estimate"] + c(-2, 2) * summary(fe_dept_model)$coefficients["scaled_rainfall_deviation", "Std. Error"]

# Checking model
fe_dept_model_2 <- lm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation + 
                        factor(COD_MUNI) + factor(year) + factor(month), 
                     data = merged_muni_data %>% filter(year != 2025))

summary(fe_dept_model_2)$coefficients["scaled_rainfall_deviation", "Estimate"]
summary(fe_dept_model_2)$coefficients["scaled_rainfall_deviation", "Estimate"] + c(-2, 2) * summary(fe_dept_model_2)$coefficients["scaled_rainfall_deviation", "Std. Error"]

# Checking model
fe_dept_model_3 <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation + scaled_temperature_deviation, 
                     data = merged_muni_data %>% filter(year != 2025), 
                     index = c("year", "month", "COD_MUNI"), 
                     model = "within")

summary(fe_dept_model_3)$coefficients["scaled_rainfall_deviation", "Estimate"]
summary(fe_dept_model_3)$coefficients["scaled_rainfall_deviation", "Estimate"] + c(-2, 2) * summary(fe_dept_model_3)$coefficients["scaled_rainfall_deviation", "Std. Error"]
...
fe_int_dept_model <- plm(scaled_homicide_deviation ~ scaled_rainfall_deviation * factor(COD_MUNI) +
                           scaled_temperature_deviation, 
                     data = merged_muni_data %>% filter(year != 2025), 
                     index = c("year", "month"), 
                     model = "within")

#saveRDS(fe_int_dept_model, here("output/fe_int_dept_model_with_temp.rds"))
#fe_int_dept_model <- readRDS(here("output/fe_int_dept_model_with_temp.rds"))
...
#write.csv(coeff_fe_int_table, here("output/coeff_fe_int_table_with_temp.csv"))
coeff_fe_int_table <- read.csv(here("output/coeff_fe_int_table_with_temp.csv"))
...
# Calculate mean rainfall and homicide deviatios for last 3 years
...
rm(fe_int_dept_model, homicide, homicide_monthly_mean,
   homicide_muni_month, homicide_muni_monthly_mean, merged_data, muni_month,
   rainfall, rainfall_muni_month, rainfall_muni_monthly_mean,
   rainfall_monthly_mean)
...
estimates_map <- ggplot(data = merged_muni_data_slopes) +
  ...
  scale_fill_viridis_c(option = "viridis", na.value = "grey80",
                       guide = guide_colourbar(title = NULL)) +
  labs(
    title = "(e) Municipality-Specific Rainfall Effects",
    fill = ""
  ) +
  theme_minimal() +
  theme(
...
period_means <- map_dfr(names(periods), function(p) {
  homicide_month %>%
    filter(year %in% periods[[p]]) %>%
    group_by(month) %>%
    summarise(mean_homicide = mean(homicide, na.rm = TRUE), .groups = "drop") %>%
    mutate(period = p)
})
...
ggsave(here('output/maps_final_5.jpg'), width = 10, height = 13)
